  ...
  <div class="max-w-6xl mx-auto p-5">
    ...
    <header v-motion="{ initial: { opacity: 0, y: -50 }, enter: { opacity: 1, y: 0, transition: { duration: 0.5 } } }"
      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-4">
          <button v-for="(label, key) in timeWindows" :key="key" v-motion="{
            initial: { scale: 1 },
            hover: { scale: 1.05 },
            tap: { scale: 0.95 }
          }" :class="[
              'px-4 py-2 border rounded transition-all duration-300 ease-in-out',
              currentWindow === key
                ? 'bg-gray-800 text-white border-gray-800 transform'
    ...
    <div v-else>
      ...
      <section class="mb-8">
        ...
        <CurrentValues :current-data="currentSensorData" />
      </section>
      ...
      <section class="mb-8" v-if="aggregatedData">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatsCard title="Temperature" :stats="aggregatedData.temperature" :has-anomaly="hasAnomalyInLastMinute('temperature')" />
          <StatsCard title="Humidity" :stats="aggregatedData.humidity" :has-anomaly="hasAnomalyInLastMinute('humidity')" />
          <StatsCard title="Air Quality" :stats="aggregatedData.air_quality" :has-anomaly="hasAnomalyInLastMinute('air_quality')" />
        </div>
      </section>
      ...
      <section class="mb-8">
        ...
        <div class="bg-white rounded-lg shadow">
          <SensorChart :data="sensorData" :time-window="currentWindow" />
        </div>
      </section>

      <!-- Error Messages -->
      <div v-if="error" class="fixed bottom-4 right-4 text-red-600 bg-red-100 p-4 rounded-lg shadow-lg border border-red-200">
...
const setupWebSocket = () => {
  ...
  ws.value.onmessage = (event: { data: string; }) => {
